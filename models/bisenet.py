"""
[1] https://github.com/wutianyiRosun/CGNet/blob/master/model/BiSeNet_xception.py
[2] https://arxiv.org/pdf/1808.00897.pdf
@misc{yu2018bisenet,
...
class ContextPath(nn.Module):
    ...
    def forward(self, x):
        ...
        up_feature_x32 = F.upsample(
            center,
            size=(self.input_h // 32, self.input_w // 32),
            mode="bilinear",
            align_corners=False,
        )
        ensemble_feature_x32 = feature_x32 + up_feature_x32

        up_feature_x16 = F.upsample(
            ensemble_feature_x32, scale_factor=2, mode="bilinear", align_corners=False
        )
        ensemble_feature_x16 = torch.cat((feature_x16, up_feature_x16), dim=1)

        up_feature_x8 = F.upsample(
            ensemble_feature_x16, scale_factor=2, mode="bilinear", align_corners=False
        )
...
class ContextPathXception(nn.Module):
    def __init__(self, input_h, input_w):
        super().__init__()
        self.input_h = input_h
        self.input_w = input_w
...
class BiSeNet_Xception34(nn.Module):
    def __init__(self, input_h, input_w, n_classes=19):
        super().__init__()
        self.spatial_path = SpatialPath()
        self.context_path = ContextPathXception(input_h, input_w)
