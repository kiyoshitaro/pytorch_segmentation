"""
[1] https://github.com/huaifeng1993/DFANet/blob/master/model/dfanet.py
[2] https://arxiv.org/pdf/1904.02216.pdf
@misc{li2019dfanet,
...
class SeparableConv2d(nn.Module):
    def __init__(
        ...
        self.pointwise = nn.Conv2d(
            inputChannel, outputChannel, 1, 1, 0, 1, 1, bias=bias
        )
...
class Block(nn.Module):
    ...
    def __init__(self, inputChannel, outputChannel, stride=1, BatchNorm=nn.BatchNorm2d):
        ...
        self.conv2 = nn.Sequential(
            SeparableConv2d(outputChannel // 4, outputChannel // 4),
            BatchNorm(outputChannel // 4),
            nn.ReLU(),
        )
        self.conv3 = nn.Sequential(
            SeparableConv2d(outputChannel // 4, outputChannel),
            BatchNorm(outputChannel),
            nn.ReLU(),
        )
        self.projection = nn.Conv2d(
            inputChannel, outputChannel, 1, stride=stride, bias=False
        )
...
class XceptionA(nn.Module):
    ...
    def __init__(self, channel_cfg, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(192, 1000), nn.ReLU(inplace=True), nn.Linear(1000, num_classes)
        )
...
class DFA_Decoder(nn.Module):
    ...
    def __init__(self, decode_channels, num_classes):
        ...
        self.conv0 = nn.Sequential(
            nn.Conv2d(
                in_channels=48, out_channels=decode_channels, kernel_size=1, bias=False
            ),
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(
                in_channels=192, out_channels=decode_channels, kernel_size=1, bias=False
            ),
            nn.BatchNorm2d(decode_channels),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x0, x1, x2, x3, x4, x5):
        ...
        x0 = self.conv0(x0)
        x1 = F.interpolate(
            self.conv1(x1), x0.size()[2:], mode="bilinear", align_corners=True
        )
        x2 = F.interpolate(
            self.conv2(x2), x0.size()[2:], mode="bilinear", align_corners=True
        )
        x3 = F.interpolate(
            self.conv3(x3), x0.size()[2:], mode="bilinear", align_corners=True
        )
        x4 = F.interpolate(
            self.conv5(x4), x0.size()[2:], mode="bilinear", align_corners=True
        )
        x5 = F.interpolate(
            self.conv5(x5), x0.size()[2:], mode="bilinear", align_corners=True
        )
